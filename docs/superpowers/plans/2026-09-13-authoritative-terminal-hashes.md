...
- [ ] On terminal materialization failure, keep the original primary error and persist accumulated frame traces with native terminal hashes explicitly unavailable. Mark the failed lane's checkpoint unavailable without invoking its materialization again; attempt independent checkpoints for every other available lane through existing guarded paths. Never fall back to stale hashes, retry the failed download, reset/restore, bypass poison/ownership, or replace the primary error with a writer/cleanup failure. Add authentic failing regressions asserting persisted traces, unavailable-state flags, no repeated failed-lane synchronization and every other lane's checkpoint attempt, for both an existing interruption and a newly primary download error.
